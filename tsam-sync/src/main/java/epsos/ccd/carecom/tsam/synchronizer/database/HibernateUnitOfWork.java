...
public abstract class HibernateUnitOfWork {
	...
	public int executeUnitOfWork() {
		...
		try {
			...
			if (this.registeredEntity != null) {
				ApplicationController.LOG.severe(this.registeredEntity.toString());
			}
			...
			if (session != null && session.isOpen()) {
				try {
					...
					ApplicationController.LOG.log(Level.WARNING, "Could not close Hibernate Session.",e);
				}
			}
		}
		return result;
	...
	protected void logDuplicateRecord(String objectName, Long objectId) {
		if (ApplicationController.LOG.isLoggable(Level.FINE)) {
			ApplicationController.LOG.fine(
	...
	protected void logTruncateString(String objectName, String fieldName, int originalFieldLength, int truncatedFieldLemgth) {
		if (ApplicationController.LOG.isLoggable(Level.INFO)) {
