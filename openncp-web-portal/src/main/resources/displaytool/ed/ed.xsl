...
<xsl:import href="edHeader.xsl"/>
<xsl:import href="edPrescriptionItem.xsl"/>
<xsl:import href="edPrescriberDetails.xsl"/>
...
<xsl:variable
			name="entryNodeED"
			select="//n1:entry/n1:supply[n1:templateId[@root='1.3.6.1.4.1.12559.11.10.1.3.1.3.3']]"/>                      
...
<xsl:template name="firstRow">
	<table>
		<tr>
			<td><xsl:call-template name="prefixCell"/></td>
		</tr>
	</table>
</xsl:template>
		...
		<xsl:call-template name="edHeader"/>
		...
		<table style="width : 100%" cols="3">
			...
			<!--
		<xsl:call-template name="edPrescriberDetails"/>			
        
		<xsl:call-template name="edPrescriptionItem"/>		
		-->
</xsl:template>
