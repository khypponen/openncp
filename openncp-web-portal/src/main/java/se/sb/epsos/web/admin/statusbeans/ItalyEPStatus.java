 ...
 */
@Component
public class ItalyEPStatus implements NcpStatusInterface, Serializable {

    private static final long serialVersionUID = 534156317345064660L;
    private static final Logger LOGGER = LoggerFactory.getLogger(ItalyEPStatus.class);
    ...
    public Boolean getStatus() {
        ...
        try {
            ...
            patientValueList.add(new PatientIdVO("patient.search.fiscal.code domain", "2.16.840.1.113883.2.9.4.3.2", "NTSTNT80B05H501F"));
            patientValueList.add(new PatientIdVO("EIHC serial number domain", "2.16.840.1.113883.2.9.4.1.4", "64731"));
            List<Person> persons = serviceFacade.queryForPatient(null, patientValueList, new CountryVO("IT", "Italien", "2.16.17.710.806.1000.990.1"));
            if (persons.size() > 0) {

                serviceFacade.setTRCAssertion(new TRC(persons.get(0), TRC.TrcPurpose.EMERGENCY), session.getUserDetails());

                CdaDocument ep = NcpStatusHelper.getEpForPerson(serviceFacade, persons.get(0));
                if (ep != null) {
                    return true;
                } else {
                    return false;
