...
public class EventIdentificationType implements Serializable {
    ...
    private static final String SERIALIZE_DATE_PATTERN = "yyyyMMddhhmmssSSS";
    ...
    private void setEventTypeCode(List<CodedValueType> eventTypeCode) {
	...
	private void writeObject(java.io.ObjectOutputStream out) throws IOException {
	    DateFormat dateFormat = new SimpleDateFormat(SERIALIZE_DATE_PATTERN);
		out.writeObject( getEventID() );
		out.writeObject( getEventTypeCode() );
		out.writeObject( getEventActionCode() );
		GregorianCalendar g = getEventDateTime().toGregorianCalendar();
		Date date = g.getTime();
		out.writeObject( dateFormat.format(date) );
		out.writeObject( getEventOutcomeIndicator() );
	}

	private void readObject(java.io.ObjectInputStream in) throws IOException, ClassNotFoundException {
	    DateFormat dateFormat = new SimpleDateFormat(SERIALIZE_DATE_PATTERN);
		setEventID( (CodedValueType)in.readObject() );
		setEventTypeCode( (List<CodedValueType>)in.readObject() );
		setEventActionCode( (String)in.readObject() );
		String sDate = (String)in.readObject();
		try {
			Date date = dateFormat.parse(sDate);
			GregorianCalendar g = new GregorianCalendar();
			g.setTime(date);
			setEventDateTime( DatatypeFactory.newInstance().newXMLGregorianCalendar(g) );
		} catch (Exception e) {
			throw new IOException(e.getMessage(), e);
		}
		setEventOutcomeIndicator( (BigInteger)in.readObject() );
	}
