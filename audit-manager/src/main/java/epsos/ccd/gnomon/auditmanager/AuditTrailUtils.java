...
import net.RFC3881.AuditSourceIdentificationType;
import net.RFC3881.CodedValueType;
import net.RFC3881.EventIdentificationType;
import net.RFC3881.ObjectFactory;
import net.RFC3881.ParticipantObjectIdentificationType;
import net.RFC3881.TypeValuePairType;

/**
 * This class provides methods for constructing the aufit message and for sending the syslog message to the repository
 *
 * @author Kostas Karkaletsis
 * @author Organization: Gnomon
 * @author mail:k.karkaletsis@gnomon.com.gr
 * @version 1.0, 2010, 30 Jun Provides methods for creating AuditMessage instances of the spSOS defined services, as for
 *          constructing and sending the syslog message to the repository
 */
public enum AuditTrailUtils {
  ...
  public AuditMessage createAuditMessage(EventLog eventLog) {
	...
	AuditMessage am = this.createAuditMessage(eventLog, eventLog.getEventTypeEnum());
  ...
  /**
   * Method to create the audit message to be passed to the syslog client. The audit message content is based on the
   * information with the log object given.
   *
   * @param eventLog The log object to contain the information to be logged.
   * @param eventType The type of the event, which controls how to construct the actual audit message.
   * @return the Audit Message object
   */
  protected AuditMessage createAuditMessage(EventLog eventLog, EventType eventType) {

	// Ensure the event type is the same in the original object
	eventLog.setEventType(eventType);
	AuditMessage am = new AuditMessage();
	AuditTrailUtils au = AuditTrailUtils.getInstance();
	switch (eventType) {
	case epsosCommunicationFailure: {
	  // if (eventLog.getEventType().equals("epsos-cf")) {
	  am = au.createAuditTrailForCommunicationFailure(eventLog);
	  // }
	  break;
	}
	case epsosConsentServiceDiscard: {
	  // if (eventLog.getEventType().equals("epsos-52")) {
	  am = au._CreateAuditTrailForConsentService(eventLog, "Discard");
	  // }
	  break;
	}
	case epsosConsentServicePin: {
	  // if (eventLog.getEventType().equals("epsos-53")) {
	  am = au._CreateAuditTrailForConsentService(eventLog, "Pin");
	  // }
	  break;
	}
	case epsosConsentServicePut: {
	  // if (eventLog.getEventType().equals("epsos-51")) {
	  am = au._CreateAuditTrailForConsentService(eventLog, "Put");
	  // }
	  break;
	}
	case epsosDispensationServiceDiscard: {
	  // if (eventLog.getEventType().equals("epsos-42")) {
	  am = au._CreateAuditTrailForDispensationService(eventLog, "Discard");
	  // }
	  break;
	}
	case epsosDispensationServiceInitialize: {
	...
	default: {
	  Object[] args = { eventType.getCode() };
	  log.error(String
	      .format("The event type '%s' could not be recognized. There was no audit message content generated.", args));
	  break;
	}
	}
  ...
  private AuditMessage _CreateAuditTrailForEhealthSMPQuery(EventLog eventLog) {
	...
	addEventTarget(am, eventLog.getET_ObjectID(), new Short("2"), null, "SMP", "eHealth Security",
  ...
  /**
   * Constructs an Audit Message for the epSOS Order Service According schema is HCP Assurcance
   *
   * @param eventLog the EventLog object
   * @return the created AuditMessage object
   */
  ...
  /**
   * Constructs an Audit Message for HCP Identity According schema is HCP Assurcance
   *
   * @param eventLog the EventLog object
   * @return the created AuditMessage object
   */
  ...
  /**
   * Constructs an Audit Message for NCP Trusted Service List According schema is HCP Assurcance
   *
   * @param eventLog the EventLog object
   * @return the created AuditMessage object
   */
  ...
  /**
   * Constructs an Audit Message for Pivot Translation According schema is HCP Assurcance
   *
   * @param eventLog the EventLog object
   * @return the created AuditMessage object
   */
  private AuditMessage _CreateAuditTrailPivotTranslation(EventLog eventLog) {
	...
	addEventTarget(am, eventLog.getET_ObjectID(), new Short("4"), new Short("5"), "in", "epSOS Tranlation",
	    "Input Data");
	addEventTarget(am, eventLog.getET_ObjectID_additional(), new Short("4"), new Short("5"), "out", "epSOS Tranlation",
	    "Output Data");
	return am;
  ...
  /**
   * Constructs an Audit Message for the epSOS Dispensation Service According schema is HCP Assurcance
   *
   * @param eventLog the EventLog object
  ...
  private AuditMessage _CreateAuditTrailForDispensationService(EventLog eventLog, String action) {
	...
	if (action.equals("Discard")) {
	  addEventTarget(am, eventLog.getET_ObjectID(), new Short("2"), new Short("4"), "12", "Discard", new Short("14"));
	} else {
	  addEventTarget(am, eventLog.getET_ObjectID(), new Short("2"), new Short("4"), "12", "", new Short("0"));
	}
  ...
  /**
   * Constructs an Audit Message for the epSOS HCER Service According schema is HCP Assurcance
   *
   * @param eventLog the EventLog object
   * @param action the action of the service
   * @return the created AuditMessage object
  ...
  /**
   * Constructs an Audit Message for the epSOS Consent Service According schema is HCP Assurcance
   *
   * @param eventLog the EventLog object
   * @param action the action of the service
   * @return the created AuditMessage object
  ...
  private AuditMessage _CreateAuditTrailForConsentService(EventLog eventLog, String action) {
	...
	AuditMessage am = createAuditTrailForHCPAssurance(eventLog);
	if (action.equals("Discard")) {
	  addEventTarget(am, eventLog.getET_ObjectID(), new Short("2"), new Short("4"), "12", action, new Short("14"));
	}
  ...
  private String getMappedEventType(String eventType) {
	...
	if (eventType
	    .equals(epsos.ccd.gnomon.auditmanager.EventType.epsosIdentificationServiceFindIdentityByTraits.getCode())) {
  ...
  private String getMappedTransactionName(String name) {
	...
	if (name.equals(
	    epsos.ccd.gnomon.auditmanager.TransactionName.epsosIdentificationServiceFindIdentityByTraits.getCode())) {
	  return epsos.ccd.gnomon.auditmanager.IHETransactionName.epsosIdentificationServiceFindIdentityByTraits.getCode();
	}
  ...
  private AuditMessage addParticipantObject(AuditMessage am, String PS_PatricipantObjectID, Short PS_TypeCode,
      Short PS_TypeRole, String PS_Name, String PS_ObjectCode, String PS_ObjectCodeName, String PS_ObjectCodeValue) {
  ...
  private AuditMessage createAuditTrailForEhealthSMPQuery(EventLog eventLog) {
	...
	try {
	  ...
	  addEventIdentification(am, eventLog.getEventType(), eventLog.getEI_TransactionName(),
	      eventLog.getEI_EventActionCode(), eventLog.getEI_EventDateTime(), eventLog.getEI_EventOutcomeIndicator());
  ...
  private AuditMessage createAuditTrailForHCPAssurance(EventLog eventLog) {
	...
	try {
	  ...
	  addEventIdentification(am, eventLog.getEventType(), eventLog.getEI_TransactionName(),
	      eventLog.getEI_EventActionCode(), eventLog.getEI_EventDateTime(), eventLog.getEI_EventOutcomeIndicator());
	  addPointOfCare(am, eventLog.getPC_UserID(), eventLog.getPC_RoleID(), true, "1.3.6.1.4.1.12559.11.10.1.3.2.2.2");
	  addHumanRequestor(am, eventLog.getHR_UserID(), eventLog.getHR_AlternativeUserID(), eventLog.getHR_RoleID(), true);
	  addService(am, eventLog.getSC_UserID(), true, "ServiceConsumer", "epSOS", "epSOS Service Consumer",
	      eventLog.getSourceip());
	  addService(am, eventLog.getSP_UserID(), false, "ServiceProvider", "epSOS", "epSOS Service Provider",
	      eventLog.getTargetip());
	  addAuditSource(am, eventLog.getAS_AuditSourceId());
	  addParticipantObject(am, eventLog.getPT_PatricipantObjectID(), new Short("1"), new Short("1"), "Patient", "2",
	      "RFC-3881", "Patient Number");
	  addError(am, eventLog.getEM_PatricipantObjectID(), eventLog.getEM_PatricipantObjectDetail(), new Short("2"),
	      new Short("3"), "9", "errormsg");
	} catch (Exception e) {
	  log.error(e.getLocalizedMessage(), e);
  ...
  private AuditMessage createAuditTrailForPatientMapping(EventLog eventLog) {
	...
	try {
	  ...
	  addService(am, eventLog.getSP_UserID(), false, "MasterPatientIndex", "epSOS", "Master Patient Index",
	      eventLog.getTargetip());
	  addAuditSource(am, eventLog.getAS_AuditSourceId());
	  addParticipantObject(am, eventLog.getPS_PatricipantObjectID(), new Short("1"), new Short("1"), "PatientSource",
	      "2", "RFC-3881", "Patient Number");
	  addParticipantObject(am, eventLog.getPT_PatricipantObjectID(), new Short("1"), new Short("1"), "PatientTarget",
	      "2", "RFC-3881", "Patient Number");
	  addError(am, eventLog.getEM_PatricipantObjectID(), eventLog.getEM_PatricipantObjectDetail(), new Short("2"),
	      new Short("3"), "9", "errormsg");
	...
	if (wta.equals("true")) {
	  ...
	  try {
		Utils.writeXMLToFile(auditmsg,
		    tap + (auditmessage.getEventIdentification().getEventTypeCode().get(0).getDisplayName().split("::"))[0]
		        + "-" + new SimpleDateFormat("yyyy.MM.dd'at'HH-mm-ss.SSS").format(new Date(System.currentTimeMillis()))
		        + ".xml");
	  } catch (Exception e) {
		try {
		  Utils.writeXMLToFile(auditmsg,
		      tap + new SimpleDateFormat("yyyy.MM.dd'at'HH-mm-ss.SSS").format(new Date(System.currentTimeMillis()))
		          + ".xml");
		} catch (Exception e2) {
  ...
  public synchronized static String constructMessage(AuditMessage auditmessage, boolean sign) {
	...
	if (validated || forcewrite) {
	  ...
	  try {
		...
		log.debug(auditmessage.getEventIdentification().getEventID().getCode() + " XML stuff: Create Dom From String");
		...
		auditmsg = "";
		log.error(auditmessage.getEventIdentification().getEventID().getCode() + " Error signing doc" + e.getMessage(),
		    e);
	  }
	}
	return auditmsg;
  ...
  /**
   * The method converts the audit message to xml format, having as input the Audit Message. Uses the JAXB library to
   * marshal the audti message object
   *
   * @param am
   * @return
